            ...
            <>
              ...
              {!isStreaming && (
                <div className="flex flex-col items-center gap-4">
                  ...
                  <div className="text-gray-500 text-xl font-medium tracking-wide">Connecting to AI Engine...</div>
                </div>
              )}
            </>
          )}
              ...
              {detections.length > 0 ? (
                <ul className="space-y-3">
                  {detections.map((det, idx) => (
                    <li key={idx} className="bg-gray-700 p-3 rounded-lg border-l-4 border-yellow-500 flex justify-between items-center">
                      <span className="font-bold text-yellow-400 text-sm">
                        [{det.id !== -1 ? `ID:${det.id}` : 'Detected'}] {det.class_name}
                      </span>
                      <span className="bg-gray-900 px-2 py-1 rounded text-xs text-green-400">{det.confidence}%</span>
                    </li>
                  ))}
                </ul>
